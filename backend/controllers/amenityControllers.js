...
const amenityControllers = {
    ...
    updateAmenityInUseStatus: async (req, res) => {
        ...
        try {
            ...
            res.status(200).json({ message: 'Cập nhật trạng thái tiện nghi thành công' });
        } catch (error) {
            res.status(500).json({ message: 'Đã xảy ra lỗi khi cập nhật trạng thái tiện nghi', error: error.message });
        }
    },
